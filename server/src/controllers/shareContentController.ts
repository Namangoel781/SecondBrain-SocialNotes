...
export const getSharedContentsByUser = async (req: Request<{ userId: string }>, res: Response) => {
    ...
    try {
        ...
        // Fetch shared content for user
        const sharedContents = await SharedContent.find({ ownerId: userId })
            .populate("tags")
            .lean();

        if (!sharedContents.length) {
            return res.status(404).json({ message: "No shared content found." });
        }
